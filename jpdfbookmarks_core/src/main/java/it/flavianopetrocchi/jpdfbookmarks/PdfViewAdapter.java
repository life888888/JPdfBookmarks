...
//		viewport.setBorder(BorderFactory.createLoweredBevelBorder());
    ...
    public void goToPage(int numPage) {
        ...
        try {
            currentPageObject = pageProducer.getPage(currentPage + 1);
        } catch (Exception e) {
...
//					scale = Math.min(scaleWidth, scaleHeight);
    ...
    private Dimension calcViewSize() {
        PageDimension scaledMediaBox = currentPageObject.getSize(scale);
    ...
    private void adjustViewportPosition() {
        PageDimension mediaBox = currentPageObject.getSize(scale);
